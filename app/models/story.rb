class Story < ActiveRecord::Base
  ...
  named_scope :with_unseen_comments_for_user, lambda { |user| {
       ...
       :joins => "INNER JOIN users ON comments.user_id = users.id INNER JOIN comments ON comments.story_id = stories.id INNER JOIN comments others_comments ON others_comments.story_id = stories.id", 
  ...
  named_scope :new_comments_for_user_stories, lambda {|user| {
      :select => "DISTINCT stories.*", 
      :joins => "INNER JOIN users ON comments.user_id = users.id INNER JOIN comments others_comments ON others_comments.story_id = stories.id",
      :conditions => ["users.id = ? AND (others_comments.created_at > users.last_login_at)", user]
    }
  }
