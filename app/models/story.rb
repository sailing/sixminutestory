class Story < ActiveRecord::Base
  ...
  named_scope :recent, lambda { { :conditions => ['created_at > ?', 1.week.ago], :order => 'created_at DESC' } }
  named_scope :popular, lambda { { :conditions => ['(comments_count >= ? or rating >= ?) AND created_at > ?', 0, 0, 1.month.ago], :order => 'counter DESC, rating DESC, updated_at ASC' } }
  named_scope :commented, lambda { { :conditions => ['(comments_count >= ?) AND created_at > ?', 1, 1.week.ago], :order => 'comments_count DESC, rating DESC, counter DESC, updated_at ASC' } }
  ...
  named_scope :with_comments_for_user, lambda { |user| {
       :select => "DISTINCT stories.*", 
       :joins => "INNER JOIN comments ON comments.story_id = stories.id INNER JOIN comments others_comments ON others_comments.story_id = stories.id INNER JOIN users ON comments.user_id = users.id", 
